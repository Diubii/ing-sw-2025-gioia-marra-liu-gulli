...
public class BuildingController extends GenericGamePhaseSceneController {
            ...
            for(int i = 0; i< mymodel.getPlayerInfos().size(); i++){
                ...
                if(mymodel.getPlayerInfos().get(i).getNickName().equals(mymodel.getMyInfo().getNickName())){
                    ...
                    switch (mymodel.getPlayerInfos().get(i).getColor()){
                        ...
                        case GREEN ->  avatars.get(j).setImage(new Image(getClass().getResource("/it/polimi/ingsw/galaxytrucker/galaxy_trucker_imgs/pedineSegnalini/AvatarVerde.png").toExternalForm()));
                    }
                    j++;
                }
            }
    ...
    /**
     * Asks the client controller to draw a random tile
     * @param actionEvent
     */
    ...
    /**
     * Adds an overlay on the center menu to let the user know he is waiting other players
     * @param clearLast
     */
    @Override
    ...
    /**
     * Activated when the finish building button is pressed asks the client controller to handle this choice
     * @param e
     */
    ...
    /**
     * Changes the interface and this class setup to reflect that the building phase is over
     */
        ...
        switch(state){
            case SHIP_CHECK:
                Platform.runLater(() -> {
                    ...
                    try {
                        //1-Prima caricare FXML
                        loader = new FXMLLoader(getClass().getResource("/it/polimi/ingsw/galaxytrucker/GuiPages/Elements/SMCheckShip.fxml"));
                        root = loader.load();
                        //2-Poi imposare il Cotnroller se ne ha bisogno passando ad esempio il controller principale o lo stage o altro
            ...
            case CREW_INIT:
                Platform.runLater(() -> {
                    ...
                    try {
                        //1-Prima caricare FXML
                        loader = new FXMLLoader(getClass().getResource("/it/polimi/ingsw/galaxytrucker/GuiPages/Elements/SMLoadCrew.fxml"));
                        root = loader.load();
                        //2-Poi imposare il Cotnroller se ne ha bisogno passando ad esempio il controller principale o lo stage o altro
    ...
    public void viewMazzo(int num){
        ...
        if(clientController.hasTileInHand() == false && clientController.viewAdventureCardDeck(num)){
            Platform.runLater(() -> {
                ...
                try {
                    //1-Prima caricare FXML
                    loader = new FXMLLoader(getClass().getResource("/it/polimi/ingsw/galaxytrucker/GuiPages/Elements/SMSpiedCards.fxml"));
                    root = loader.load();
                    //2-Poi imposare il Cotnroller se ne ha bisogno passando ad esempio il controller principale o lo stage o altro
